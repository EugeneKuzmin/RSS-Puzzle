...
function transformArray(
  array: ObjectWithLevelDataAndWords
): { words: string[] }[] {
...
function getPuzzleContainer(): HTMLFormElement {
  const puzzleContainer = new BaseHTMLElementClass('div', ['puzzle-container']);

  return puzzleContainer.getElement() as HTMLFormElement;
}
function getPickWordContainer(): HTMLFormElement {
  const pickWordContainer = new BaseHTMLElementClass('div', [
    'pickword-container',
    'container'
  ]);
...
export default function createMainPage(): HTMLFormElement {
  const wrapper = new BaseHTMLElementClass(
    'div',
    ['flex', 'align-items-center', 'flex-column'],
    'English puzzle'
  );
  ...
  wrapper.appendChilds([puzzleConrainer, pickWordConrainer]);
